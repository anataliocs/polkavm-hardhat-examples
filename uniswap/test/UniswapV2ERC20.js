...
describe('UniswapV2ERC20', function () {
    ...
    it("name, symbol, decimals, totalSupply, balanceOf, DOMAIN_SEPARATOR, PERMIT_TYPEHASH", async () => {
        ...
        (0, chai_1.expect)(await token.symbol()).to.eq("UNI-V2");
        (0, chai_1.expect)(await token.decimals()).to.eq(18);
        (0, chai_1.expect)(await token.totalSupply()).to.eq(TOTAL_SUPPLY);
        (0, chai_1.expect)(await token.balanceOf(deployer)).to.eq(TOTAL_SUPPLY);
        const tokenAddress = await token.waitForDeployment();
        const { chainId } = await hardhat_1.ethers.provider.getNetwork();
        (0, chai_1.expect)(await token.DOMAIN_SEPARATOR()).to.eq(hardhat_1.ethers.keccak256(abiCoder.encode(["bytes32", "bytes32", "bytes32", "uint256", "address"], [
        ...
        (0, chai_1.expect)(await token.PERMIT_TYPEHASH()).to.eq(hardhat_1.ethers.keccak256(hardhat_1.ethers.toUtf8Bytes("Permit(address owner,address spender,uint256 value,uint256 nonce,uint256 deadline)")));
    });
    it("approve", async () => {
        ...
        (0, chai_1.expect)(await token.allowance(walletAddress, await other.getAddress())).to.eq(TEST_AMOUNT);
    });
    it("transfer", async () => {
        ...
        (0, chai_1.expect)(await token.balanceOf(await wallet.getAddress())).to.eq(TOTAL_SUPPLY - TEST_AMOUNT);
        (0, chai_1.expect)(await token.balanceOf(await other.getAddress())).to.eq(TEST_AMOUNT);
    });
    it("transfer:fail", async () => {
        await (0, chai_1.expect)(token.transfer(await other.getAddress(), TOTAL_SUPPLY + 1n)).to.be.reverted;
        await (0, chai_1.expect)(token.connect(other).transfer(await wallet.getAddress(), 1n)).to.be.reverted;
    });
    it("transferFrom", async () => {
        await token.approve(await other.getAddress(), TEST_AMOUNT);
        (0, chai_1.expect)(await token.allowance(await wallet.getAddress(), await other.getAddress())).to.eq(TEST_AMOUNT);
        await (0, chai_1.expect)(token
            .connect(other)
            .transferFrom(await wallet.getAddress(), await other.getAddress(), TEST_AMOUNT))
            .to.emit(token, "Transfer")
            .withArgs(await wallet.getAddress(), await other.getAddress(), TEST_AMOUNT);
        (0, chai_1.expect)(await token.allowance(await wallet.getAddress(), await other.getAddress())).to.eq(0n);
        (0, chai_1.expect)(await token.balanceOf(await wallet.getAddress())).to.eq(TOTAL_SUPPLY - TEST_AMOUNT);
        (0, chai_1.expect)(await token.balanceOf(await other.getAddress())).to.eq(TEST_AMOUNT);
    });
    it("transferFrom:max", async () => {
        ...
        await (0, chai_1.expect)(token
            ...
            .to.emit(token, "Transfer")
            .withArgs(await wallet.getAddress(), await other.getAddress(), TEST_AMOUNT);
        (0, chai_1.expect)(await token.allowance(await wallet.getAddress(), await other.getAddress())).to.eq(hardhat_1.ethers.MaxUint256);
        (0, chai_1.expect)(await token.balanceOf(await wallet.getAddress())).to.eq(TOTAL_SUPPLY - TEST_AMOUNT);
        (0, chai_1.expect)(await token.balanceOf(await other.getAddress())).to.eq(TEST_AMOUNT);
    });
});
